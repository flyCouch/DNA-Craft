# ==============================================================================
# PROJECT:   DNA_MARK_I_V47_DRO_SPACING_FIX
# THEME:     Clinical Blue / Medical White
# FOCUS:     Axis Gutter Expansion / Negative Sign Buffer / Placard Timing Lock
...
class StationData:
    def __init__(self, name):
        ...
        self.pos_x = ctk.DoubleVar(value=0.00)
        self.pos_y = ctk.DoubleVar(value=0.00)
        self.pos_z = ctk.DoubleVar(value=15.00)

class DNAMarkI(ctk.CTk):
    def __init__(self):
        super().__init__()

        # Worker Print Protocol
        print(f"Project: DNA_MARK_I_V47 | Date: {datetime.now().strftime('%Y-%m-%d')} | Time: {datetime.now().strftime('%H:%M:%S')}")
        ...
        self.cylinder_height = ctk.StringVar(value="15.50")
    ...
    def setup_ui(self):
        ...
        # DRO (Expanded to 650px for axis isolation)
        ...
        for i, (axis, var) in enumerate([("X", self.cur_x), ("Y", self.cur_y), ("Z", self.cur_z)]):
            cell = ctk.CTkFrame(dro_inner, fg_color="transparent", width=200) # MAX WIDTH CELLS
            cell.grid(row=0, column=i, padx=10) # 10px PADDING BETWEEN CELLS
        ...
        r_grid = ctk.CTkFrame(sn_box, fg_color="transparent"); r_grid.pack(expand=True, fill="both")
        r_grid.grid_columnconfigure((0,1,2,3), weight=1); r_grid.grid_rowconfigure((0,1), weight=1)
        for i, name in enumerate(STATION_NAMES):
            ctk.CTkRadioButton(r_grid, text=name.upper(), variable=self.active_station, value=name, font=("Arial", 12, "bold")).grid(row=i//4, column=i%4, sticky="nsew", padx=5)
    ...
    def open_settings(self, name):
        ...
        self.create_input(pos_box, "Y-Position (mm):", data.pos_y)
